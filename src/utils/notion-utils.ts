...
export function transformInformation(data: NotionResponse): Information[] {
  return data.results.map((item: any) => ({
    ...
    contests: item.properties.contests?.rich_text[0]?.plain_text || null,
    projects: item.properties.projects?.rich_text[0]?.plain_text || null,
    rewards: item.properties.rewards?.rich_text[0]?.plain_text || null
  }));
}