...
import { downloadAndStoreImage } from './image-utils';

export async function transformMembers(data: NotionResponse): Promise<Member[]> {
  ...
  return Promise.all(
    data.results.map(async (item: any) => {
      ...
      const originalImageUrl = returnImage ? item.properties.image?.files[0]?.file?.url || null : null;
      const localImageUrl = originalImageUrl ? await downloadAndStoreImage(originalImageUrl, 'member', item.id) : null;
...
export async function transformAwards(data: NotionResponse): Promise<Award[]> {
  return Promise.all(
    data.results.map(async (item: any) => {
      const originalImageUrl = item.properties.image?.files[0]?.file?.url || null;
      const localImageUrl = originalImageUrl ? await downloadAndStoreImage(originalImageUrl, 'award', item.id) : null;
...
export async function transformProjects(data: NotionResponse): Promise<Project[]> {
  return Promise.all(
    data.results.map(async (item: any) => {
      const originalImageUrl = item.properties.image?.files[0]?.file?.url || null;
      const localImageUrl = originalImageUrl ? await downloadAndStoreImage(originalImageUrl, 'project', item.id) : null;
